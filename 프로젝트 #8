...
#include <Servo.h> // servo 라이브러리를 사용한다.
...
/*
delay 를 주지 않으면 모터 움직임 보다 각도 데이터가 더 빠르기 때문에 데이터
버퍼링 일어나 튐 현상 발생. 꼭 딜레이를 줘야함.
*/
...
for(position = 170; position >= 10; position -= 1) //거꾸로 움
직이자
{
